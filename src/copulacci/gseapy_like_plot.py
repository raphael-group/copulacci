...
## The plotting code are 
# taken from GSEApy
# with sligh modifications
# -*- coding: utf-8 -
...
from scipalette import SciPalette
...
class DotPlot(object):
    ...
    def process(self, df: pd.DataFrame):
        ...
        # get scatter area
        if df.columns.isin([self.value_column]).any():
            ol = df.columns[df.columns.isin([self.value_column])]
    ...
    def scatter(
        ...
        ax.xaxis.set_tick_params(labelsize=self.xlabelsize)
        ax.yaxis.set_tick_params(labelsize=self.ylabelsize)
...
def dotplot(
    ...
    **kwargs,
    ...
    """
    df = df_orig.copy()
    if not(column in ['pval','qval']):
        df.loc[:, 'pval'] = df[column]
        colname = 'pval'
    ...
    dot = DotPlot(
        ...
        x=x,
        y='ligrec_dir' if show_ligrec_dir else y,
...
def pairwise_scatter(
    ...
    for gpair in res_dict:
        res = res_dict[gpair].copy()
...
def plot_lig_rec(
    ...
    lr_pairs_g1 = int_edges_new_with_selfloops.loc[
                    int_edges_new_with_selfloops.interaction == group,
                    ["cell1", "cell2"]
                ]
    x = count_df.loc[ lr_pairs_g1.cell1.values, ligand ].values.astype('int')
    y = count_df.loc[ lr_pairs_g1.cell2.values, receptor ].values.astype('int')
...
def plot_lig_rec2(
    ...
    df_tmp = pd.DataFrame({ligand:x, receptor:y})
    import seaborn as sns
    sns.jointplot(df_tmp, x= ligand, y=receptor, kind='reg')
    return (x,y)
...
def plot_lig_rec3(
    ...
    if len(ligand) > 0:
        if func == 'min':
            x = count_df.loc[ lr_pairs_g1.cell1.values, ligand ].min(axis=1).values.astype('int')
        elif func == 'mean':
            x = count_df.loc[ lr_pairs_g1.cell1.values, ligand ].mean(axis=1).values.astype('int')
        elif func == "sum":
            x = count_df.loc[ lr_pairs_g1.cell1.values, ligand ].sum(axis=1).values.astype('int')
    else:
        x = count_df.loc[ lr_pairs_g1.cell1.values, ligand ].values.astype('int') 
    
    if len(receptor) > 0:
        if func == 'min':
            y = count_df.loc[ lr_pairs_g1.cell2.values, receptor ].min(axis=1).values.astype('int')
        elif func == 'mean':
            y = count_df.loc[ lr_pairs_g1.cell2.values, receptor ].mean(axis=1).values.astype('int')
        elif func == 'sum':
            y = count_df.loc[ lr_pairs_g1.cell2.values, receptor ].sum(axis=1).values.astype('int')
    else:
        y = count_df.loc[ lr_pairs_g1.cell2.values, receptor ].values.astype('int') 
    
    df_tmp = pd.DataFrame({ligand[0]:x, receptor[0]:y})
    import seaborn as sns
    sns.jointplot(df_tmp, x= ligand[0], y=receptor[0], kind='reg')
    plt.xlabel("_".join(ligand))
    plt.ylabel("_".join(receptor))
    return (x,y)
